...
python ${stage3path}/stage.py -i ${ligand} -o ${outbasename} -v --forcefields ${ff} -q ${charge} -c ${protein} -w ${water}

cd ${outbasename}_${ff}

gmx_mpi grompp -f ${templatepath}/minim.mdp -c solvated_ionised.gro -r solvated_ionised.gro -p ${outbasename}.top -o min.tpr -po mdout.mdp
